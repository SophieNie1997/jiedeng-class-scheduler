...
test("syncs teacher roster and default shifts from the latest duty workbook", () => {
  assert.deepEqual(
    ...
    ["Phebe", "Sophie", "Lynn", "Tiana", "Catherine", "Gioia", "Karen", "Charlotte", "Hanna", "Reece"],
  );
  assert.equal(shiftRoster.some((person) => person.id === "claire"), false);
  assert.equal(shiftRoster.some((person) => person.id === "lency"), false);
  assert.equal(shiftRoster.some((person) => person.id === "vicky"), false);
  assert.equal(shiftRoster.some((person) => person.id === "reece"), true);
  assert.equal(shiftRoster.some((person) => person.id === "charlotte"), true);
  assert.equal(shiftRoster.some((person) => person.id === "gioia"), true);
  assert.equal(shiftRoster.some((person) => person.id === "karen"), true);
  assert.equal(shiftRoster.some((person) => person.id === "hanna"), true);
});

test("does not keep default shifts from files outside the selected folders", () => {
  assert.deepEqual(defaultShiftOverrides, {});
});
